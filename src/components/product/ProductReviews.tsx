  ...
  return (
    <section className="mt-20">
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-[280px_1fr]">
        {/* Summary + form */}
        <div>
          <div className="rounded-2xl border border-brand-900/8 bg-white p-6 text-center shadow-sm">
            <p className="font-display text-5xl font-extrabold text-brand-950">{avg.toFixed(1)}</p>
          ...
          <form onSubmit={submit} className="mt-4 rounded-2xl border border-brand-900/8 bg-white p-5 shadow-sm">
            <h3 className="font-display text-base font-bold text-brand-950">Write a review</h3>
            ...
            <input
              ...
              className="mt-3 w-full rounded-xl border border-brand-900/15 px-3.5 py-2.5 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20"
            />
            <textarea
              value={text}
              onChange={(e) => {
              ...
              className="mt-2 w-full resize-none rounded-xl border border-brand-900/15 px-3.5 py-2.5 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20"
            />
        ...
        <div>
          {userReviews.length === 0 ? (
            <div className="flex h-full min-h-40 flex-col items-center justify-center rounded-2xl border border-dashed border-brand-900/15 bg-white/60 p-8 text-center">
              <p className="font-display text-lg font-bold text-brand-950">Be the first to review</p>
            ...
            <ul className="space-y-4">
              {userReviews.map((r) => (
                <li key={r.id} className="rounded-2xl border border-brand-900/8 bg-white p-5 shadow-sm">
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-brand-950">{r.author}</p>
                  ...
                  {r.text && <p className="mt-3 text-sm leading-relaxed text-brand-900/75">{r.text}</p>}
                </li>
              ))}
            </ul>
          )}
