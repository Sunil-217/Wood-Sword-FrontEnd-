...
const STATUS_STYLES: Record<OrderStatus, string> = {
  ...
  Shipped: "bg-brand-600/15 text-brand-700",
    ...
    return (
      <Container className="py-20">
        <div className="mx-auto max-w-md text-center">
          ...
          <h1 className="mt-6 font-display text-2xl font-bold text-brand-950">No orders yet</h1>
...
function OrderCard({ order }: { order: Order }) {
  return (
    <div className="overflow-hidden rounded-2xl border border-brand-900/8 bg-white shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-brand-900/8 bg-brand-50/50 px-5 py-3.5">
        <div>
          <p className="font-display text-sm font-bold text-brand-950">{order.id}</p>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="font-display text-base font-bold text-brand-950">{inr(order.total)}</span>
        </div>
      </div>
      <div className="flex flex-wrap gap-4 px-5 py-4">
        {order.items.map((line) => (
          <div key={line.id} className="flex items-center gap-3">
            <span className="h-12 w-12 shrink-0 overflow-hidden rounded-lg border border-brand-900/8">
              <ProductArt art={line.art} accent={line.accent} label={line.name} className="h-full w-full" />
            </span>
            <span className="min-w-0">
              <span className="block max-w-[180px] truncate text-sm font-medium text-brand-950">{line.name}</span>
              <span className="block text-xs text-brand-900/50">Qty {line.qty}</span>
            </span>
          </div>
        ))}
      </div>
      <div className="border-t border-brand-900/8 px-5 py-3 text-xs text-brand-900/55">
